...
router.get('/', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM sapi');
...
router.post('/', async (req, res) => {
  const { nama, ras, jenis_kelamin, berat_badan, status, keterangan } = req.body;
  if (!nama || !jenis_kelamin) {
    return res.status(400).json({ status: 'error', message: 'nama dan jenis_kelamin wajib diisi' });
  }
  try {
    const [result] = await pool.query(
      'INSERT INTO sapi (nama, ras, jenis_kelamin, berat_badan, status, keterangan) VALUES (?, ?, ?, ?, ?, ?)',
      [nama, ras, jenis_kelamin, berat_badan, status || 'Sehat', keterangan]
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      'UPDATE sapi SET nama=?, ras=?, jenis_kelamin=?, berat_badan=?, status=?, keterangan=? WHERE id=?',
      [nama, ras, jenis_kelamin, berat_badan, status, keterangan, req.params.id]
    );
